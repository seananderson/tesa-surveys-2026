...
.small[
Idea: there's an underlying surface of fish population intensity. You observe it as one of: weight, count, or presence-absence.
]
...
### Multi-likelihood syntax (3 data types)
...
```{r, eval=FALSE, echo=TRUE}
unique(dat$data_type)
#> [1] "delta_lognormal" "nbinom2" "binomial" 
```
]
...
fit <- sdmTMB(
  list(
    response ~ factor(year) + factor(data_type),
    # only 1 gear type in 2nd linear predictor:
    response ~ factor(year) #<<
  ),
  family = family_list, #<<
  distribution_column = "data_type", #<<
  ...
